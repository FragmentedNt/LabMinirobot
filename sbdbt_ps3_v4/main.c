...
unsigned int sixaxis_button_state=BUTTON_STATE_INIT;
BYTE sixaxis_home;
...
#error	"Unknown format"
	...
	pos++;
#elif	FORMAT_RCB4
	lineBuffer[pos++]=(sixaxis_button_state>>0)&0xff;	//1バイト目:ボタン状態デジタル2
        lineBuffer[pos++]=(sixaxis_button_state>>8)&0xff;	//2バイト目:ボタン状態デジタル1
        lineBuffer[pos++]=(sixaxis_home)&0xff;                  //3バイト目:ボタン状態デジタル1
	...
	lineBuffer[pos++] = (lineBuffer[0] + lineBuffer[1] + lineBuffer[2] + lineBuffer[3] + lineBuffer[4] + lineBuffer[5] + lineBuffer[6] + 0x7c + 0x7c) & 0xFF;		//8バイト目:チェックサム
        lineBuffer[pos++] = 0x7C;                                 //9バイト目:チェックサム
        lineBuffer[pos] = 0x7C;                                 //10バイト目:チェックサム
	pos++;
#elif	FORMAT_RPU
